"""
   This script is used to create x_train.txt and y_train after applying stop_words, lemmatization, tokenization
"""
...
N = 20000 # number of documents to take
nlp = spacy.load('fr_core_news_lg') # load NLTK french stop words

# The list of the created stop words
sw_list2 = ["a", "abord", "afin", "ah", "ai", "qu", "aie", "ailleurs", "ainsi", "ait", "allaient","film", "allo",
...
sw_list = get_stop_words("fr") + ["c'est", "j'ai", "qu'il", "qu'elle", "qu'ils", "qu'elles", "d'un",
                                  ...
                                  "celui"] + stopwords.words('french')

# "entre", "ça", "même", "meme","mème", "scénario", "personnage", "acteur", "scène","la", "celui"
for w in sw_list:
...
def readFile(filename):
    ...
    for line in open(filename, 'r', encoding='UTF-8'):
        label = ast.literal_eval(line).get("note")
        line = ast.literal_eval(line).get("commentaire")
        labeList.append(label)
        trainList.append(line)
    return trainList, labeList


file, labelist = readFile("../data/train_comments.txt")


def tokenization(trainList):
    filtered_corpus = []
    for i in range(N):  # range(len(trainList))
        if trainList[i] is None :
            trainList[i] = "TempElement"
        word_tokens = text_to_word_sequence(unicodedata.normalize("NFKD", trainList[i]))
        lemma_word = []
        ...
        if not filtered_comment:
            filtered_comment.append("TempElement")
...
tokens_output = open('../data/nltk/x_train.txt', 'w', encoding='utf-8')
notes_output = open('../data/nltk/y_train.txt', 'w', encoding='utf-8')
for i in range(len(vector_corpus)):
    if vector_corpus[i] == "TempElement":
        pass
    else:
        print(vector_corpus[i], file=tokens_output)
        print(labelist[i], file=notes_output)
    print(i, " of ", len(vector_corpus))
